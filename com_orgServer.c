...
int main(){
	...
	struct sockaddr_in ComOrgServerAddr; 
	struct sockaddr_in LocalServerAddr;
	
	struct DNS_Header header;
	struct DNS_Query query;

	char bufout[DNS_LENGTH_MAX]; /* String to send to echo server */
	...
	ComOrgServerAddr.sin_family=AF_INET;
	ComOrgServerAddr.sin_port=htons(53);
	ComOrgServerAddr.sin_addr.s_addr=inet_addr("127.0.0.5");
	if ((bind(sock, (struct sockaddr *) &ComOrgServerAddr, sizeof(ComOrgServerAddr))) < 0)
		printf("bind() failed.\n");
...
//服务器等待客户端连接中，游客户端连接时调用accept产生一个新的套接字
	...
	while(1)/* Run forever */
	{
		...
		if ((recvMsgSize = recv(local_fd, bufin, sizeof(bufin), 0)) < 0)
			printf("recv() failed.\n");
...
//		}
		...
		if (sendto(local_fd,bufout,out-bufout+3,0, (struct sockaddr *) &LocalServerAddr,sizeof(LocalServerAddr))!=(out-bufout+3)){
