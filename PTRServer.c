...
int main(){
	...
	struct sockaddr_in govServerAddr; /* Echo server address */
	...
	govServerAddr.sin_family=AF_INET;
	govServerAddr.sin_port=htons(53);
	govServerAddr.sin_addr.s_addr=inet_addr("127.0.0.8");
	if ((bind(sock, (struct sockaddr *) &govServerAddr, sizeof(govServerAddr))) < 0)
		printf("bind() failed.\n");
...
//			}
		...
		char *filePath;
		if(resQuery.queryType==PTR_Type){
        	filePath="PTRDatabase.txt";
