...
//Í¨¹ıµ÷ÓÃlisten½«Ì×½Ó×ÖÉèÖÃÎª¼àÌıÄ£Ê½
	...
	while(1)/* Run forever */
	{
		//·şÎñÆ÷µÈ´ı¿Í»§¶ËÁ¬½ÓÖĞ£¬ÓÎ¿Í»§¶ËÁ¬½ÓÊ±µ÷ÓÃaccept²úÉúÒ»¸öĞÂµÄÌ×½Ó×Ö
...
//			}
		...
		resHead.answerNum = 0; 
		resHead.authorNum = htons(1);/*ÒòÎª»á¸æÖªÏÂÒ»¼¶·şÎñÆ÷µØÖ·,Ã»ÕÒµ½ÔòºóĞø¸ü¸ÄÎª0*/
		resHead.addNum = 0;
		resQuery = recvQuery;
...
unsigned int extractQuery(char* beginingPointer, struct DNS_Query* dnsquery){
	...
	//Íê³É±¨ÎÄÖĞÊı×Ö¼ÓÓòÃûĞÎÊ½ÖÁµã·ÖÖµµÄ×ª»» 
	while(1){
...
//	printf("Converted name: %s\n", domainName);
	...
	memcpy(dnsquery->name, domainName, i); //´ËÊ±µÄi±ãÎª±ä³¤×Ö·û´®µÄ³¤¶ÈÁË£¬¾­¹ıÁËÑ­»·±éÀú 
//	printf("Query name: %s\n", dnsquery->name);
...
//	 printf("Query Class: %d\n", dnsquery->queryClass);
	return i+4+1; //ÍøÂçĞÎÊ½µÄÓòÃû±íÊ¾ºÍµã·ÖÖµ²î1£¬ÔÚÕâÀïÌØµØ²¹ÉÏ 
}

void splitDomainName(char *domainName, char *splitName){
	int i = strlen(domainName)-1; //ÃâÈ¥\0µÄÓ°Ïì 
	//printf("domainName: %s\n", domainName);
	int j = 0;
	int k = 0;
	int countdot=0;
...
//			}			
	    ...
	    memcpy(bufout, &temp, sizeof(short));
	    bufout+=2;
	    unsigned int tempint = htonl(record->ttl); //è¿™é‡Œæ˜¯htonl 32ä½æ•°å­—çš„ä¸»æœºå­—èŠ‚åºè½¬åŒ– 
...
//        printf("ipppppppppppppppppppppppppppppaddddddddddddddddddr : %u",ipAddr);
		memcpy(bufout, &ipAddr,record->data_len); //å°†å­—ç¬¦ä¸²è½¬åŒ–ä¸ºç½‘ç»œå­—èŠ‚åºçš„4bytesæ•°æ® 
//		int p=0;
//		while(p<=50){
...
//	    }
		//printf("rrDate: %s\n", o);
		bufout+=record->data_len; //ä¹Ÿå°±æ˜¯è¦ç§»åŠ¨4ä½ 
	    //printf("i=%d\n", i);
	    //temp =  htons(49164); //è¿™é‡ŒæŒ‡ä»£1100000000001100ï¼ŒDNSæŠ¥æ–‡ä¸­å‹ç¼©æŒ‡é’ˆçš„æ“ä½œ
